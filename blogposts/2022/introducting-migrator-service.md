...
heroImage: https://sourcegraphstatic.com/blog/introducing-migrator-service/introducing-migrator-service.jpg
socialImage: https://sourcegraphstatic.com/blog/introducing-migrator-service/introducing-migrator-service.jpg
published: true
description: "We have identified and solved a huge source of pain during upgrades for Sourcegraph administrators. Here's how."
---

We have recently identified the root cause of a very frequent and insidious pain related to database migrations during Sourcegraph version upgrades. Many users handling the installation and upgrade of their Sourcegraph instance can likely attest to seeing an error similar to the following mid-way through an upgrade from one minor release to the next.
...
2. We see this issue frequently in our Cloud environment, but exclusively on objects over a particular (but undetermined) size, such as our repository or code intelligence data tables.
...
After a second or so, the Postgres deadlock detector kicks in, notices two processes vying for each other's locks, and cancels the deadlocked index creation operation. This error is a bit dense, but states that the index creation operation is waiting for **Process B**'s attempt to acquire the advisory lock to finish, but the advisory lock acquisition is blocked on **Process A**'s ownership of that lock. Concurrent index creation failures are also a paritcularly nasty failure mode, as they leave the database with a _partially created_ index that exists in the system catalog but is accessible neither for reads nor writes. To resolve this database state, the invalid index has to be dropped and re-created. This is an easy step to forget, if required to be done manually, and forgetting the step can have very severe performance penalties if not detected.
