...
# Added
  ...
  - description: "Search Notebooks now support importing and exporting Markdown-formatted files."
    ...
    category: Search
# Changed
  - description: Syntax highlighting for JSON now uses a distinct color for strings in object key positions.
    url: https://github.com/sourcegraph/sourcegraph/pull/30105
    category:
  - description: "GraphQL API: The order of events returned by `MonitorTriggerEventConnection` has been reversed so newer events are returned first. The `after` parameter has been modified accordingly to return events older the one specified, to allow for pagination. TODO"
    ...
    category: API
  - description: "[Query based search contexts](https://docs.sourcegraph.com/code_search/how-to/search_contexts#beta-query-based-search-contexts) are now enabled by default as a [beta feature](https://docs.sourcegraph.com/admin/beta_and_experimental_features)."
    url: https://github.com/sourcegraph/sourcegraph/pull/30888
    category: Search
# Fixed
  - description: Links generated by editor endpoint will render image preview correctly.
    url: https://github.com/sourcegraph/sourcegraph/pull/30767
    category:
  - description: Fixed a race condition in the precise code intel upload expirer process that prematurely expired new uploads.
    ...
    category:
  - description: "Pushing changesets from Batch Changes to code hosts with self-signed TLS certificates has been fixed."
    url: https://github.com/sourcegraph/sourcegraph/issues/31010
    category: Admin
  - description: Fixed LSIF uploads not being expired according to retention policies when the repository contained tags and branches with the same name but pointing to different commits.
    url: https://github.com/sourcegraph/sourcegraph/pull/31108
    category: Repositories
  - description: Service discovery for the symbols service can transition from no endpoints to endpoints. Previously we always returned an error after the first empty state.
    url: https://github.com/sourcegraph/sourcegraph/pull/31225
    category:
