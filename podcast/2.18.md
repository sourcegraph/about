...
* Wed Feb 23 2022 12:04:30 GMT-0800 (PST)
* Source doc: Kelly Norton Transcript_Ritza
----->
...
So yeah, we worked directly with them. This is actually how I ended up getting into web performance tools because, at the time, everybody's problem was that there were mysterious performance gremlins in their apps. Usually, the problems were on Internet Explorer, but not always. So I transitioned naturally from working on GWC mostly to basically building a bunch of hacked-together tooling to find performance issues in all of the various browsers. This is also how I got really interested in hacking on browsers in general.
...
We would set aside JS Prime and bring some things we learned to it into the early versions of Dart. And we would continue to move down this path where the folks in Seattle and Atlanta were really wanting, "All right, whatever we do, we're creating a VM over here, and we're making a language to compile from the web here. Let's don't compromise each other. Like, please don't do anything that would make it hard for us to compile to JavaScript."
