...
We've updated the JSON configuration format so it now requires a slash command. Now the command is defined by the JSON object key rather than in a special field. We removed the old `slashCommand` field and added a new `description` field that describes the usage for each command.
