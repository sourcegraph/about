...
- We require passing checks on GitHub PRs before merging (and don't allow direct pushes to master). Sometimes it's nice to push without waiting for checks (such as for docs-only changes), but this is outweighed by the downside that people too often accidentally merged changes that broke the build. We intend to allow certain kinds of changes (such as docs-only changes) to bypass the full build pipeline so the checks pass quickly in those cases.