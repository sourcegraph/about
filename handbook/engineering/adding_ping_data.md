...
5. Test the new schema. Schedule a meeting with @EricBM for this part. 
  ...
  - Make a query to the target table to see if it worked. There may be a short period after the schema change that causes a message to dead-letter, so it may be helpful to also check the [error records table](https://bigquery.cloud.google.com/table/telligentsourcegraph:sourcegraph_analytics.update_checks_test_error_records?pli=1).
