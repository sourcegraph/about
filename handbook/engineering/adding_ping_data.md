...
5. Test the new schema
 	- Delete the [test table](https://bigquery.cloud.google.com/table/telligentsourcegraph:sourcegraph_analytics.update_checks_test?pli=1) (`$DATASET.$TABLE_backup`) and create a new table, with the same name so it doesn't break Pub/Sub, with the update schema (commands are below). 
