...
## How to create a license key for a new customer

First, the customer's Sourcegraph administrator must create a Sourcegraph.com user account and provide their username. Once that is available,follow the steps below.
...
- Set the licensed number of users (note that if you added the `true-up` tag above, the customer will be able to exceed this count, but administrators will see a warning) and the number of days that the license should be valid, and click **Generate license**.
