...
Welcome to Sourcegraph, to the Customer Engineering department, and the Customer Support team! This document will guide you through customer support-specific onboarding. You’ll start the tasks listed here after you complete the company onboarding checklist, as this builds on our company onboarding and focuses on everything you need to be successful in this particular role.

Our onboarding is centered on outcomes: With X time interval, we expect Y outcome. In support of this, each time interval has a task list. If you complete the tasks and you do not feel the outcomes to be true, that likely indicates something needs to be improved about the onboarding. Make note of it, share it with the team, and we will figure out how to make sure you feel the outcomes to be true and improve the checklist for the next new team member. At this point, it’s not you. This onboarding plan is new and requires so much improvement.
...
* Verify you have access to all the required tools -- more on all of these later
	...
	* Calendly (if you would like to use it when you need to find a time with a customer to troubleshoot together)
	* Salesforce (our source of truth for non-ping generated customer data)
	* Looker (where we visualize customer data)
...
* Verify you are in the following Slack channels (and join them if not) -- if you have any questions about the purpose or value of a particular channel (not all of them have updated descriptions or topics), ask in #customer-support-chat. It’s a lot of channels and can feel super overwhelming. Sometimes it helps to use Slack’s section functionality so you can organize all of them by those you need to pay attention to more frequently vs those you can look at whenever.
	* Team channels
		...
		* docs: This is a channel run by product (they own our official documentation), but we will likely be in here a lot collaborating as we start to help with docs
...
* Get a Sourcegraph instance [deployed](https://docs.sourcegraph.com/admin/install) using the `docker-compose` flow. This will be your own test environment. If you get stuck, just post in our #customer-support-chat Slack channel to start and if we get stuck as a team, we will see if our CE counterparts can help by asking them in #ce-chat.
* Watch [Dave’s Kubernetes Walkthrough video](https://drive.google.com/drive/folders/1JqQlrmGX8u_EuhNptj-qrzuZ4RifhTuF) for an understanding of that setup process, too.
* Set up your [local development environment](https://github.com/sourcegraph/sourcegraph/blob/main/doc/dev/getting-started/index.md). If you get stuck, just post in our #customer-support-chat Slack channel to start and if we get stuck as a team, we will get help in #any-questions. You will often want to run Sourcegraph locally when you are troubleshooting.
...
* You have time scheduled with each engineering manager and each member of the Distribution team
* You have signed-up for which playbook(s) you will help with and have a plan for how to approach it
...
Important slack channels and fun ones too
IDE's used (kinda falls under common tools)
Security / privacy guidelines when speaking to customers. 
Example: Do we need to confirm who we are talking to is a Sourcegraph customer? This topic can get lengthy, but kinda falls under scope of support / due diligence.
Who is who, and teams we'll be interacting with most often
Escalation points of contact if we get stuck on a technical problem (engineering?) or if a customer is getting spicy (customer engineers?) and the processes for communicating with them
Guidelines for taking a break / FTO / what to do if we need to go to a Dr. appt, etc
Support engineer metrics for how well we're doing. How do we measure success?
Who do we talk to about Feature requests, either on behalf of customers or because we see a need that customers are not articulating?
Heads down (project) time, what that will look like

As far as the onboarding plan, are there sections that the plan will be broken into that you can think of right now? At this moment, I’d say I’d love to see how you think would be the best way to go about our day to day as a CSE, as well as examples of customer concerns/ questions, and some more details into the product. Outside of that, I can’t think of anything in which I don’t think you would have already covered, but I will start to think of those examples now and provide you with that information as it comes up. 
