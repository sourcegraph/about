...
- [Sourcegraph equity FAQ](equity-faq.md)
...
However, managers can and do occasionally grant ad hoc bonuses (cash and equity are both possible) for strong performance.

We believe that reoccurring bonuses without clear and consistent targets can introduce a bias that affects goal-setting and behavior, so we prefer salary-based compensation by default.
...
- The latest raw data for relevant roles is copied into each team's [compensation bands spreadsheets](#compensation-bands). This automatatically will update the band data.
  - Bands are calculated using:
    ...
    - The 75th percentile gross equity value (in US dollar value) is used to back into a count of options.
- The bands are shared with each department lead, who has the ability to manually adjust bands where desired (for example, to interpolate where the data is rough or based on a low sample size, to create bands for roles that don't have corresponding raw data, to make adjustments for their particular hiring needs based on their experience, etc.).
- The Talent and People Ops teams review and sign off on the final approved numbers.

### Making out-of-band offers

Bands are resources for hiring managers to use—they are not intended to be permanent or set in stone. Unique situations may exist where bands don't make sense for various reasons, from lack of granularity to cover a niche role to bad data. 

Hiring managers that want to make offers outside of the 75th to 90th percentile cash compensation or outside of +/- 5% of the gross value of the options must receive approval from their department heads and the People Ops team.
...
Full-time contractors are compensated using the same bands as employees, and we provide [location-independent compensation](#location-independent).

All contractors [submit invoices monthly](../invoices.md), while employees are paid twice per month (on the 15th and the last day of the month if those are weekdays, or the Fridays before those days if not).

Full-time contractors are eligible for all [perks](../benefits-and-perks.md) that employees are eligible for, limited where necessary by law (for example, Sourcegraph cannot provide medical insurance to non-employees). Given tax rates and insurance subsidies vary widely by country, we do not provide a blanket benefit for contractors to make up for this.
