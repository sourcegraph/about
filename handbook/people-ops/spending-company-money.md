...
- We have two separate "all-in" budgets (in USD), one for full-time teammates and one for interns:
  - The budget for interns is $500 and includes only the electronic items in the "Desk set-up expenses" section (with the exception of laptops, as these are separate from the all-in budget).
  - The budget for full-time teammates is $1000 for your desk setup, which includes all the items in the "Desk set-up expenses" section (with the exception of laptops, as these are separate from the all-in budget). If you expect to significantly exceed the listed amount on any purchase, please notify @sourcegraph/people-ops so we can decide if we need to update this guide.
