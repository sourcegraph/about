...
1. **Understand where customers falloff during setup process**
   1. Time to important admin setup milestones (i.e. instance started, admin account created, >1 repositories added) are tracked in [pings](https://docs.sourcegraph.com/admin/pings).
...
1. **LSIF is adopted by customers**
   ...
   1. At least one customer has > 75% of code intel queries powered by LSIF (excluding fallback references)
