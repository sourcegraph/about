...
1. **LSIF is adopted by customers**
   1. At least three customers have > 50% of code intel queries powered by LSIF (excluding fallback references) => Not complete, see Notes below.
   1. At least one customer has > 90% of code intel queries powered by LSIF (excluding fallback references) => Not complete, see Notes below.
   1. 99th percentile end-to-end latency of hover tooltips, go to definition, and find references is tracked in [pings](https://docs.sourcegraph.com/admin/pings) and under 2 seconds. These metrics are inclusive of all of these user actions, regardless of how code intelligence is implemented. => Pings are [implemented](https://github.com/sourcegraph/sourcegraph/pull/8423), and a [prototype Looker dashboard](https://sourcegraph.looker.com/dashboards/131) exists.
...
- **Notes:** We modified Objective (1) to focus on [making LSIF adoption more attractive](https://docs.google.com/document/d/1J0wtYEF2gkfxPfNhMjQKulfKI7eb1Jc_NRuGDpMGpDs/edit) to customers, by:
    - Building a showcase of precise code intelligence, using public code indexed on sourcegraph.com (see [RFC 122](https://docs.google.com/document/d/1W8uR0vQBYtdkNynMxEq24CFdzPTWkGijCURsosdvzAM/edit)), and
    - Fixing operational issues that hinder LSIF deployment at scale (see [RFC 127](https://docs.google.com/document/d/1lno7N_gDg2RmvI4cLZ7l_TdnxP1e-P1Ri7Iwr2RCjKQ/edit)).
