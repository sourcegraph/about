...
1. **Transparent and scalable code-host permissions syncing**
   1. 95th percentile latency of authorization on the read path (i.e. `authzFilter`) is below 10ms. => Prometheus metrics in place, not able to add to ping because of technical diffcuities. Instead, will add alerts to Grafana panels.
